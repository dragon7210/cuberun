...
function ShipModel(props, { children }) {
  ...
  useFrame((state, delta) => {
    ...
    outerConeExhaust.current.material.map.offset.y +=
      0.01 * (0.4 + mutation.gameSpeed) * delta * 165;
    coneExhaust.current.material.map.offset.y -=
      0.01 * (0.4 + mutation.gameSpeed) * delta * 165;
    ...
    const scaleFactor =
      mutation.currentMusicLevel > 0.8 ? mutation.currentMusicLevel + 0.2 : 1;

    innerConeExhaust.current.scale.z = fastSine / 15;
    innerConeExhaust.current.scale.x =
      (innerConeScaleFactor.current + fastSine / 15) * scaleFactor;
